from database.querys import extraccion_ids_hs, insert_datos
from hubspot_extract import disponibilidad_por_AdId, ventas_por_AdId, chequeos_de_credito_por_AdId, ventas_de_others_companies

filters = [
    {
    "filtros": [{ 
        "pagina": "AT&T-Hs",
        "facebook_page": ['Home Services (Provider)'],
        "filter": [{
        "fibra": ["Fiber"],   
        "dsl": ["25-75 Mb", "100"],   
...
def home_services (date_start, date_stop, date_start_original, date_stop_original):
    ad_id = extraccion_ids_hs(date_start_original,date_stop_original)
                ...
                for type in tipo[0]:
                    try:
                        if type.find("sales") != 0:
                            ...
                            insert_datos(type,result,"Home Services",id[1],date_start_original,date_stop_original)
                            # print("Total:",result)
                        else:
            ...
            if pagina == "Credit Check":
                for type_credit in tipo[0]:
                    try:
                        ...
                        insert_datos(type_credit,result,"Home Services",id[1],date_start_original,date_stop_original)
                        # print("Total:",result)
                    except:
                        print()
